...
const TABLE_HEAD = [
  ...
  { id: "name", label: "SupplierName", alignRight: false },
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(
      array,
      (_supplier) => _supplier.name.toLowerCase().indexOf(query.toLowerCase()) !== -1
    );
  }
    ...
    try {
      await axios.get(`${API_URL}/api/suppliers`).then((response)=> {
        if(response.data.status){
          setSupplierRows(response.data.results);
...
const handleSelectAllClick = (event) => {
  if (event.target.checked) {
    const newSelecteds = supplierRows.map((n) => n.city_id);
...
const emptyRows =
  page > 0 ? Math.max(0, (1 + page) * rowsPerPage - supplierRows.length) : 0;
 ...
 const breadcrumbs = [
  ...
  <Typography key="3" color='text.disabled'>
    ...
    return (
        <>
      <Helmet>
        <title> Supplier List | ITAM </title>
      </Helmet>
...
{!(filteredCities.length > 0) && !(isNotFound) && (
                ...
                {filteredCities
                  ...
                  .map((row) => {
                    ...
                    const selectedCity = selected.indexOf(id) !== -1;
                      ...
                      >
                        ...
                        <TableCell align="right">
                          <IconButton
                            // size="large"
                            color="inherit"
                            onClick={(e)=>{handleOpenMenu(e,supplier_id)}}
                          >
...
<TablePagination
      ...
      >
        <MenuItem onClick={()=>{handleClickEdit(supplierId);handleCloseMenu();}}>
          <Iconify icon={"eva:edit-fill"} sx={{ mr: 2 }} />
          Edit
        </MenuItem>

        <MenuItem sx={{ color: "error.main" }} onClick={()=>{handleClickDelete(supplierId);handleCloseMenu();}}>
