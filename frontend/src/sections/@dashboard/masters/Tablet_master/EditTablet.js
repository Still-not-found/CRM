...
export default function EditTablet(props) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        await axios.get(`${API_URL}/api/tablets/${idToEdit}`).then((response) => {
          if (response.data.status) {
            const { assigned_qty, warrenty, tablet_name, location, company, order_number, purchase_date, purchase_cost, brand, model, imei, os, serial_number, screen_size, current_owner, warrenty_expiry_date, color, storage_capacity, status_id, comments , supplier_id,} = response.data.results[0];
            setTabletData({
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.put(`${API_URL}/api/tablets/${idToEdit}`, { ...tabletData, purchaseDate: dayjs(tabletData.purchaseDate).format("YYYY-MM-DD"),warrenty: dayjs(tabletData.warrenty).format("YYYY-MM-DD"), updatedBy: loggedUser.user_id }).then((response) => {
  ...
  return (
    <>
      <Helmet>
        <title>Tablet Edit | IT Asset Management</title>
      </Helmet>
