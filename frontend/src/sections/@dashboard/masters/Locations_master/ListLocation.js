...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(
      array,
      (_location) => _location.name.toLowerCase().indexOf(query.toLowerCase()) !== -1
    );
  }
    ...
    try {
      await axios.get(`${API_URL}/api/locations`).then((response)=> {
        if(response.data.status){
          setLocationRows(response.data.results);
...
const handleSelectAllClick = (event) => {
  if (event.target.checked) {
    const newSelecteds = locationRows.map((n) => n.city_id);
...
const emptyRows =
  page > 0 ? Math.max(0, (1 + page) * rowsPerPage - locationRows.length) : 0;
 ...
 const breadcrumbs = [
  ...
  <Typography key="3" color='text.disabled'>
    ...
    return (
        <>
      <Helmet>
        <title> Location List | ITAM </title>
      </Helmet>
                      ...
                      >
                        ...
                        <TableCell align="right">
                          <IconButton
                            // size="large"
                            color="inherit"
                            onClick={(e)=>{handleOpenMenu(e,location_id)}}
                          >
...
<TablePagination
      ...
      >
        <MenuItem onClick={()=>{handleClickEdit(locationId);handleCloseMenu();}}>
          <Iconify icon={"eva:edit-fill"} sx={{ mr: 2 }} />
          Edit
        </MenuItem>

        <MenuItem sx={{ color: "error.main" }} onClick={()=>{handleClickDelete(locationId);handleCloseMenu();}}>
