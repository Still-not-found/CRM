...
export default function ViewAccesspoint(props) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        await axios.get(`${API_URL}/api/accesspoints/${idToView}`).then((response) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.put(`${API_URL}/api/accesspoints/${idToView}`, { ...accesspointData, purchaseDate: dayjs(accesspointData.originalinstalldate).format("YYYY-MM-DD"), updatedBy: loggedUser.user_id }).then((response) => {
  ...
  return (
    <>
      ...
      >
        <DialogTitle>{"View Accesspoint"}</DialogTitle>
