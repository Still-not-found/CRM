...
export default function ViewWorkstation(props) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        await axios.get(`${API_URL}/api/workstations/${idToView}`).then((response) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.put(`${API_URL}/api/workstations/${idToView}`, { ...workstationData, originalinstalldate: dayjs(workstationData.originalinstalldate).format("YYYY-MM-DD"), updatedBy: loggedUser.user_id }).then((response) => {
  ...
  return (
    <>
      <Helmet>
        <title>Workstation View | IT Asset Management</title>
      </Helmet>
      ...
      >
        <DialogTitle>{workstationData.workstationName}</DialogTitle>
