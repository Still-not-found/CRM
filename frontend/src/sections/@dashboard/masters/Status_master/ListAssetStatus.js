...
const TABLE_HEAD = [
  ...
  { id: "status_name", label: "Status Name", alignRight: false },
  { id: "status_code", label: "Status Code", alignRight: false },
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(
      array,
      (_status) => _status.status_name.toLowerCase().indexOf(query.toLowerCase()) !== -1
    );
  }
    ...
    try {
      await axios.get(`${API_URL}/api/status`).then((response)=> {
        if(response.data.status){
          setStatusRows(response.data.results);
...
const handleSelectAllClick = (event) => {
  if (event.target.checked) {
    const newSelecteds = statusRows.map((n) => n.status_id);
...
const emptyRows =
  page > 0 ? Math.max(0, (1 + page) * rowsPerPage - statusRows.length) : 0;
...
{!(filteredCities.length > 0) && !(isNotFound) && (
                ...
                {filteredCities
                  ...
                  .map((row) => {
                    ...
                    const selectedStatus = selected.indexOf(status_id) !== -1;
                      ...
                      >
                        ...
                        <TableCell align="right">
                          <IconButton
                            // size="large"
                            color="inherit"
                            onClick={(e)=>{handleOpenMenu(e,status_id)}}
                          >
...
<TablePagination
      ...
      >
        <MenuItem onClick={()=>{handleClickEdit(statusId);handleCloseMenu();}}>
          <Iconify icon={"eva:edit-fill"} sx={{ mr: 2 }} />
          Edit
        </MenuItem>

        <MenuItem sx={{ color: "error.main" }} onClick={()=>{handleClickDelete(statusId);handleCloseMenu();}}>
