...
export default function EditStatus(props) {
  ...
  useEffect(()=>{
    ...
    const fetchData = async() =>{
      try {
        await axios.get(`${API_URL}/api/status/${idToEdit}`).then((response)=>{
          if(response.data.status){
            const {status_name, status_code} = response.data.results[0];
            setStatusData({
  ...
  const handleSubmit = async (event)=> {
    ...
    try {
      await axios.put(`${API_URL}/api/status/${idToEdit}`,{...statusData,updatedBy:loggedUser.user_id}).then((response)=>{
