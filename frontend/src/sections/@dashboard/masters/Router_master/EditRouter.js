...
export default function EditNRouter(props) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        await axios.get(`${API_URL}/api/nrouters/${idToEdit}`).then((response) => {
          if (response.data.status) {
            const { assigned_qty, warrenty, nrouter_name, location, company, order_number, purchase_date, purchase_cost, brand, model, ip_address, subnetmask, gateway, firmware, installation_date, last_maintenance_date, status_id, comments , supplier_id,} = response.data.results[0];
            setNRouterData({
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.put(`${API_URL}/api/nrouters/${idToEdit}`, { ...nrouterData, purchaseDate: dayjs(nrouterData.purchaseDate).format("YYYY-MM-DD"),lastMaintenanceDate: dayjs(nrouterData.lastMaintenanceDate).format("YYYY-MM-DD"),installationDate: dayjs(nrouterData.installationDate).format("YYYY-MM-DD"),warrenty: dayjs(nrouterData.warrenty).format("YYYY-MM-DD"), updatedBy: loggedUser.user_id }).then((response) => {
  ...
  return (
    <>
      <Helmet>
        <title>NRouter Edit | IT Asset Management</title>
      </Helmet>
      ...
      >
        <DialogTitle>{"Edit"  + "-" + nrouterData.nrouterName}</DialogTitle>
                ...
                <Grid item xs={4} sm={12} md={12}>
                  ...
                  <LocalizationProvider
                    ...
                    <DemoContainer components={['DatePicker']}>
                      ...
                      format="DD/MM/YYYY"
                      value={nrouterData.lastMaintenanceDate}
                      onChange={(newValue)=>
                ...
                <Grid item xs={4} sm={12} md={12}>
                  <LocalizationProvider
                    ...
                    <DemoContainer components={['DatePicker']}>
                      ...
                      format="DD/MM/YYYY"
                      value={nrouterData.installationDate}
                      onChange={(newValue)=>
              ...
              <Grid item xs={4} sm={12} md={12}>
                  <LocalizationProvider
                    ...
                    <DemoContainer components={['DatePicker']}>
                      ...
                      format="DD/MM/YYYY"
                      value={nrouterData.purchaseDate}
                      onChange={(newValue)=>
