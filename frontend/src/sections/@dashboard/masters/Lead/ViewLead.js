...
export default function ViewLead(props) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        await axios.get(`${API_URL}/api/leads/${idToView}`).then((response) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.put(`${API_URL}/api/leads/${idToView}`, { ...leadData, originalinstalldate: dayjs(leadData.originalinstalldate).format("YYYY-MM-DD"), updatedBy: loggedUser.user_id }).then((response) => {
