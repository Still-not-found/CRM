...
export default function EditInventory(props) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.put(`${API_URL}/api/computers/${idToEdit}`, { ...computerData, originalinstalldate: dayjs(computerData.originalinstalldate).format("YYYY-MM-DD"), updatedBy: loggedUser.user_id }).then((response) => {
  ...
  return (
    <>
      <Helmet>
        <title>Inventory Edit | IT Asset Management</title>
      </Helmet>
                  ...
                  >
                    ...
                    {/* <Grid item xs={4} sm={12} md={12}>
                      <TextField
                        ...
                        required
                        multiline={true}
                        rows={computerData.installedsoftware.split(',').length}
