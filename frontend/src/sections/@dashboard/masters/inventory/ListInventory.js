...
const TABLE_HEAD = [
  { id: "name", label: "Inventory Name", alignRight: false },
...
export default function ListInventory(props){
 ...
 const [computerRows, setInventoryRows]=useState([]);

 const [computerId, setInventoryId] = useState(null);
    ...
    try {
      await axios.get(`${API_URL}/api/computers`).then((response)=> {
        if(response.data.status){
          setInventoryRows(response.data.results);
        }
      }).catch((error)=> {
        setInventoryRows([]);
        setStatus({
...
const filteredInventorys = applySortFilter(
  computerRows,
  getComparator(order, orderBy),
  filterName
);

const isNotFound = !filteredInventorys.length && !!filterName;
 ...
 const breadcrumbs = [
  ...
  <Typography key="3" color='text.disabled'>
    ...
    return (
      ...
      <Helmet>
        <title> Inventory List | IT Asset Management </title>
      </Helmet>
...
<TableBody>
{!(filteredInventorys.length > 0) && !(isNotFound) && (
                ...
                {filteredInventorys
                  ...
                  .map((row) => {
                    ...
                    const selectedInventory = selected.indexOf(computer_id) !== -1;
