...
export default function EditInvoice(props) {
  ...
  const softwareDetailsArray = invoiceData.installedsoftware.split('", "');
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        await axios.get(`${API_URL}/api/invoices/${idToEdit}`).then((response) => {
  ...
  const validate = () => {
    ...
    const handleViewInvoice = () => {
      const invoiceUrl = `${API_URL}/uploads/${invoiceData.invoice}`;
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.put(`${API_URL}/api/invoices/${idToEdit}`, { ...invoiceData, purchasedate: dayjs(invoiceData.purchasedate).format("YYYY-MM-DD"),warrenty: dayjs(invoiceData.warrenty).format("YYYY-MM-DD"), updatedBy: loggedUser.user_id }).then((response) => {
  ...
  return (
    <>
      <Helmet>
        <title>Invoice Edit | IT Asset Management</title>
      </Helmet>
                  ...
                  >
                    <Grid item xs={4} sm={12} md={12}>
                      <Typography variant="body1" disabled="true">
                        {invoiceData.installedsoftware.split(',').map((item, index) => (
                          <div key={index}>
      ...
      <Grid container columnSpacing={3}>
        <Grid item xs={4} sm={12} md={12}>
          ...
          >
            <Grid item xs={4} sm={12} md={12}>
              {invoiceData.installedsoftware.split(',').map((item, index, array) => (
                <React.Fragment key={index}>
        ...
        InputProps={{
          endAdornment: (
            <InputAdornment position="end">
              <IconButton
                onClick={()=>{window.open(`${API_URL}/uploads/${invoiceData.invoice}`, '_blank')}}
                edge="end"
              >
