...
const TABLE_HEAD = [
  { id: "name", label: "Invoice Name", alignRight: false },
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(
      array,
      (_invoice) => _invoice.name.toLowerCase().indexOf(query.toLowerCase()) !== -1
    );
  }
...
export default function ListInvoice(props){
 ...
 useEffect(()=>{
  const fetchData= async()=>{
    ...
    try {
      await axios.get(`${API_URL}/api/invoices`).then((response)=> {
        if(response.data.status){
          setInvoiceRows(response.data.results);
        }
      }).catch((error)=> {
        setInvoiceRows([]);
        setStatus({
 ...
 const handleOpenMenu = (event,invoice_id) => {
...
const handleSelectAllClick = (event) => {
  if (event.target.checked) {
    const newSelecteds = invoiceRows.map((n) => n.invoice_id);
 ...
 const breadcrumbs = [
  ...
  <Typography key="3" color='text.disabled'>
    ...
    return (
      ...
      <Helmet>
        <title> Invoice List | IT Asset Management </title>
      </Helmet>
...
<TableBody>
{!(filteredInvoices.length > 0) && !(isNotFound) && (
                ...
                {filteredInvoices
                  ...
                  .map((row) => {
                    ...
                    const selectedInvoice = selected.indexOf(invoice_id) !== -1;
                    ...
                    return (
                      ...
                      >
                        ...
                        }} align="left" onClick={()=>{navigate(`/invoice_detail/${invoice_id}`)}}>
                            {name}
                        </TableCell>
                        ...
                        <TableCell align="right">
                          <IconButton
                            color="inherit"
                            onClick={(e)=>{handleOpenMenu(e,invoice_id)}}
                          >
...
<TablePagination
      ...
      >
        <MenuItem onClick={()=>{navigate(`/invoice_detail/${invoiceId}`)}}>
          <Iconify icon={"eva:eye-fill"} sx={{ mr: 2 }} />
          View
        </MenuItem>

         <MenuItem onClick={()=>{handleClickEdit(invoiceId);handleCloseMenu();}}>
          <Iconify icon={"eva:edit-fill"} sx={{ mr: 2 }} />
          Edit
        </MenuItem>

        <MenuItem sx={{ color: "error.main" }} onClick={()=>{handleClickDelete(invoiceId);handleCloseMenu();}}>
