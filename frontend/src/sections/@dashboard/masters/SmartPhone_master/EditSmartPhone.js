...
export default function EditSmartPhone(props) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        await axios.get(`${API_URL}/api/smartphones/${idToEdit}`).then((response) => {
          if (response.data.status) {
            const { assigned_qty, smartphone_name, warrenty, location, company, order_number, purchase_date, purchase_cost, brand, model, imei, os, serial_number, screen_size, current_owner, warrenty_expiry_date, color, storage_capacity, status_id, comments , supplier_id,} = response.data.results[0];
            setSmartPhoneData({
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.put(`${API_URL}/api/smartphones/${idToEdit}`, { ...smartphoneData, purchaseDate: dayjs(smartphoneData.purchaseDate).format("YYYY-MM-DD"),warrenty: dayjs(smartphoneData.warrenty).format("YYYY-MM-DD"), updatedBy: loggedUser.user_id }).then((response) => {
  ...
  return (
    <>
      <Helmet>
        <title>SmartPhone Edit | IT Asset Management</title>
      </Helmet>
      ...
      >
        <DialogTitle>{"Edit SmartPhone"}</DialogTitle>
              ...
              <Grid item xs={4} sm={12} md={12}>
                  <LocalizationProvider
                    ...
                    <DemoContainer components={['DatePicker']}>
                      ...
                      format="DD/MM/YYYY"
                      value={smartphoneData.purchaseDate}
                      onChange={(newValue)=>
