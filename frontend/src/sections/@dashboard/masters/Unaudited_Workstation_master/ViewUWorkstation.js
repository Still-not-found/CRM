...
export default function ViewUWorkstation(props) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        await axios.get(`${API_URL}/api/uworkstations/${idToView}`).then((response) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.put(`${API_URL}/api/uworkstations/${idToView}`, { ...uworkstationData, originalinstalldate: dayjs(uworkstationData.originalinstalldate).format("YYYY-MM-DD"), updatedBy: loggedUser.user_id }).then((response) => {
  ...
  return (
    <>
      <Helmet>
        <title>UWorkstation View | IT Asset Management</title>
      </Helmet>
      ...
      >
        <DialogTitle>{"View UWorkstation"}</DialogTitle>
