...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(
      array,
      (_company) => _company.company_name.toLowerCase().indexOf(query.toLowerCase()) !== -1
    );
  }
    ...
    try {
      await axios.get(`${API_URL}/api/companies`).then((response)=> {
        if(response.data.status){
          setCompanyRows(response.data.results);
...
const handleSelectAllClick = (event) => {
  if (event.target.checked) {
    const newSelecteds = companyRows.map((n) => n.company_id);
...
const emptyRows =
  page > 0 ? Math.max(0, (1 + page) * rowsPerPage - companyRows.length) : 0;
                      ...
                      >
                        ...
                        <TableCell align="right">
                          <IconButton
                            // size="large"
                            color="inherit"
                            onClick={(e)=>{handleOpenMenu(e,company_id)}}
                          >
...
<TablePagination
      ...
      >
        <MenuItem onClick={()=>{handleClickEdit(companyId);handleCloseMenu();}}>
          <Iconify icon={"eva:edit-fill"} sx={{ mr: 2 }} />
          Edit
        </MenuItem>

        <MenuItem sx={{ color: "error.main" }} onClick={()=>{handleClickDelete(companyId);handleCloseMenu();}}>
