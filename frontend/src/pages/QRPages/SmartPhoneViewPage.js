...
export default function SmartPhoneViewPage(props) {
...
// console.log(smartphoneId);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        await axios.get(`${API_URL}/api/smartphones/${smartphoneId}`).then((response) => {
  ...
  const handleClose = (event, reason) => {
    if (reason && (reason == "backdropClick" || reason == "escapeKeyDown"))
      return;
      navigate('/app/asset_management/smartphone');
  };
...
// Save or display the PDF document as needed
  ...
  return (
    <>
      ...
      >
        <DialogTitle>{smartphoneData.smartphoneName}</DialogTitle>
  ...
  <Grid container spacing={3}>
    ...
    <Grid item xs={4} sm={12} md={12}>
      ...
      <Grid container columnSpacing={3} justifyContent="center">
        <Grid item>
          <QRCode id="qr-code" value={`http://localhost:3000/smartphone_detail/${smartphoneId}`} />
        </Grid>
        <Grid item>
          <Button variant="contained" onClick={() => downloadQR(smartphoneData.smartphoneName)}>Download QR</Button>
        </Grid>
        <Grid item>
          <Button variant="contained" onClick={()=>printQR(smartphoneData.smartphoneName)}>Print QR</Button>
        </Grid>
      </Grid>
    </Box>
            {/* <Box flexGrow={1}>
              ...
              <QRCode value={`http://localhost:3000/smartphone_detail/${smartphoneId}`}/>
              </Grid>
            </Box> */}
          </TabPanel>
