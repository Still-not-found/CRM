...
export default function PrinterViewPage(props) {
...
// console.log(printerId);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        await axios.get(`${API_URL}/api/printers/${printerId}`).then((response) => {
  ...
  const handleClose = (event, reason) => {
    if (reason && (reason == "backdropClick" || reason == "escapeKeyDown"))
      return;
      navigate('/app/asset_management/printer');
  };
...
// Save or display the PDF document as needed
  ...
  const validate = () => {
    ...
    if (!Boolean(printerData.printerName))
  ...
  return (
    <>
      ...
      >
        <DialogTitle>{printerData.printerName}</DialogTitle>
      ...
      <Grid container columnSpacing={3} justifyContent="center">
        <Grid item>
          <QRCode id="qr-code" value={`http://localhost:3000/printer_detail/${printerId}`} />
        </Grid>
        <Grid item>
          <Button variant="contained" onClick={() => downloadQR(printerData.printer)}>Download QR</Button>
        </Grid>
        <Grid item>
          <Button variant="contained" onClick={()=>printQR(printerData.printer)}>Print QR</Button>
        </Grid>
      </Grid>
    </Box>
            {/* <Box flexGrow={1}>
              ...
              <QRCode value={`http://localhost:3000/printer_detail/${printerId}`}/>
              </Grid>
            </Box> */}
          </TabPanel>
