...
export default function NRouterViewPage(props) {
  // const { optionState, handleClickView, setRefresh, setStatus, idToView, loggedUser } = props;
const navigate =useNavigate();
const {nrouterId}= useParams();
// console.log(nrouterId);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        await axios.get(`${API_URL}/api/nrouters/${nrouterId}`).then((response) => {
  ...
  const handleClose = (event, reason) => {
    if (reason && (reason == "backdropClick" || reason == "escapeKeyDown"))
      return;
      navigate('/app/asset_management/router');
  };
  ...
  return (
    <>
      ...
      <Grid container columnSpacing={3} justifyContent="center">
        <Grid item>
          <QRCode id="qr-code" value={`http://localhost:3000/nrouter_detail/${nrouterId}`} />
        </Grid>
        <Grid item>
          <Button variant="contained" onClick={() => downloadQR(nrouterData.nrouterName)}>Download QR</Button>
        </Grid>
        <Grid item>
          <Button variant="contained" onClick={()=>printQR(nrouterData.nrouterName)}>Print QR</Button>
        </Grid>
      </Grid>
    </Box>
            {/* <Box flexGrow={1}>
              ...
              <QRCode value={`http://localhost:3000/nrouter_detail/${nrouterId}`}/>
              </Grid>
            </Box> */}
          </TabPanel>
